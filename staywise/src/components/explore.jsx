...
const Explore = () => {
  ...
  const logInteraction = async (action, motelIds) => {
    try {
      ...
      const response = await axios.post("http://54.234.37.135:5000/log-interaction", payload);
...
const fetchRecommendations = async (userId, city, state) => {
  try {
      ...
      const response = await axios.get(`http://54.234.37.135:5000/recommend-motels/${userId}`);
 ...
 // Mapping of state names to abbreviations
  ...
  useEffect(() => {
    ...
    const fetchMotels = async () => {
      try {
        const state = 'NY'
        const response = await axios.get(`http://54.234.37.135:5000/motels?state=${state}`);
        // Fetch average ratings for each motel
        const ratings = {};
        for (const motel of response.data) {
          const ratingResponse = await axios.get(`http://54.234.37.135:5000/reviews/averages/${motel.id}`);
  ...
  const handleLocationSearch = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://54.234.37.135:5000/locationSearch?state=${stateAbbreviation}&city=${city}`
      );
